...
bool functionTransformReorder(std::unordered_map<llvm::Function*, int>& Worklist,
                              llvm::SmallVector<llvm::Function*, 8>& OrderedFunc,
                              llvm::Function* Func) {
  errs() << "Adding " << Func->getName() << " to the list.\n";
  Worklist.erase(Func);
  OrderedFunc.push_back(Func);
  for (inst_iterator I = inst_begin(Func), E = inst_end(Func); I != E; ++I) {
    Instruction &Inst = *I;
    if (CallBase *CB = dyn_cast<CallBase>(&Inst)) {
      if (Function *CF = CB->getCalledFunction()) {
        if (Worklist.find(CF) != Worklist.end()) {
          errs() << "Adding " << CF->getName() << " to the list.\n";
          Worklist.erase(CF);
          OrderedFunc.push_back(CF);
          functionTransformReorder(Worklist, OrderedFunc, CF);
        }
      }
    }
  }
...
bool BlindedInstrConversionPass::runImpl(Function &F,
  ...
  MadeChange |= expandBlindedArrayAccesses(F, AA, TaintInfo[&F]);
...
PreservedAnalyses BlindedInstrConversionPass::runAnalysis(Function &F,
                                                  FunctionAnalysisManager &AM,
                                                  SmallSet<Function *, 8> &VisitedFunctions) {
  ...
  bool MadeChange = false;
  propagateBlindedArgumentFunctionCalls(F, AAResult, TR, AM, VisitedFunctions);
  MadeChange = markBlindedIfNecessary(F, AAResult, TR);

  VisitedFunctions.erase(&F);
  TaintInfo[&F] = TR.getTaintedRegisters(&AAResult);
...
PreservedAnalyses BlindedInstrConversionPass::taintTrackingAll(Module& M, ModuleAnalysisManager &AM) {
  ...
  while (!FunctionWorkList.empty()) {
    ...
    PreservedAnalyses PassPA;
    {
      ...
      PassPA = runAnalysis(*F, FAM, VisitedFunctions);
    }
...
PreservedAnalyses BlindedInstrConversionPass::run(Module &M,
  ...
  PreservedAnalyses PA = PreservedAnalyses::all();
  PreservedAnalyses TPA = taintTrackingAll(M, AM);
  PA.intersect(std::move(TPA));

  // while (!FunctionWorkList.empty()) {
  for (auto FAPair : TaintInfo) {

    Function *F = FAPair.first;
    FunctionWorkList.pop_back();
